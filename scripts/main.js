...
const toggleTheme = () => {
  ...
  if (currentPage === "detail.html") {
    ...
    buttons.forEach(
      (btn) => (btn.style.border = isDarkMode ? "1px solid #7bffe9" : "none")
    );
  } else {
    const imgUrl = document.querySelectorAll("#image-detail");

    imgUrl.forEach((img) => (img.style.color = isDarkMode ? "white" : "black"));
    submitBtn.style.border = isDarkMode ? "1px solid #7bffe9" : "none";
  }

  body.style.backgroundColor = isDarkMode ? "#2a3b49" : "white";
